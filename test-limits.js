...
const { canGenerateReport, canAddClient, getPlanLimits } = require('./src/lib/plan-limits')
...
async function testLimits() {
  ...
  try {
      ...
      if (limitCheck.allowed) {
        ...
        await prisma.report.create({
          data: {
            ...
            pdfUrl: 'test.pdf',
            userId: testUser.id,
            clientId: null // No client required for this test
          }
        })
      } else {
    ...
    const report26Check = await canGenerateReport(testUser.id)
    console.log(`STARTER Report 26: allowed=${report26Check.allowed}, current=${report26Check.currentCount}, limit=${report26Check.limit}`)
